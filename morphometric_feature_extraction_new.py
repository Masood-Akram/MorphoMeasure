...
# ======================= Argument Parsing =======================
parser = argparse.ArgumentParser(description="Extract morphometrics from SWC files using L-Measure")
parser.add_argument('--tag', type=float, default=3.0,
                    help='Branch tag to use for morphometric extraction (default: 3.0). Use 7.0 for axon-tagged files.')
...
for swc_filename in os.listdir(swc_dir):
    if swc_filename.endswith(".swc"):
        ...
        for feature_name, feature_flags in features.items():
            # ==== Insert tag into the command ====
            feature_flags_formatted = (
                feature_flags.replace("{TAG}", str(branch_tag))
